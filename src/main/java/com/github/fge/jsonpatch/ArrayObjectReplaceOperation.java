 ...
 * value.</p>
 */
public final class ArrayObjectReplaceOperation extends PathOldNewValueOperation {
	@JsonCreator
	public ArrayObjectReplaceOperation(@JsonProperty(JsonDiffConstants.PATH) final JsonPointer path, @JsonProperty(JsonDiffConstants.OLD_VALUE) final JsonNode oldValue,
			@JsonProperty(JsonDiffConstants.NEW_VALUE) final JsonNode newValue) {
		super(JsonDiffConstants.REPLACE, path, oldValue, newValue);
	}
	...
	public JsonNode apply(final JsonNode node) throws JsonPatchException {
		...
		final JsonNode replacement = oldValue.deepCopy();
		if (path.isEmpty())
			return replacement;
